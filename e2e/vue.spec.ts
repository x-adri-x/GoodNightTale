...
test.describe('Test Initialize Screen', () => {
  ...
  test.skip('click on Generate Tale button navigates to content screen', async ({ page }) => {
    await expect(
    ...
    await expect(page.url()).toBe('http://localhost:5173/content')
  })
})
...
test.describe('Test Content Screen', () => {
  ...
  test.fixme('toast message is shown when request fails', async () => {})
})
